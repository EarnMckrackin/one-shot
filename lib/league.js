...
export async function fetchWeeklyReleases(dateStr) {
  ...
  for (const provider of providers) {
    try {
      ...
      // We want at least a decent number of releases to consider it a success for "weekly releases"
      // If it's very few, maybe it's an incomplete data set for that provider
      if (Array.isArray(releases) && (releases.length > 5 || (provider.name === "League of Comic Geeks" && releases.length > 0))) {
        return {
          releases,
