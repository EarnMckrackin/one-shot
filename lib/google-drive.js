...
const DRIVE_SEARCH_STOP_WORDS = new Set(["and", "the", "for", "from", "with", "into", "vol", "volume"]);

export async function listPDFsInFolder(drive, folderId, search = "", { includeFullText = false } = {}) {
  const baseQuery = `'${escapeDriveQueryValue(folderId)}' in parents and mimeType='application/pdf' and trashed=false`;
  const terms = String(search)
    .split(/\s+/)
    .map(term => term.trim().replace(/^[^\w#]+|[^\w]+$/g, ""))
    .filter(term => (term.length >= 3 || /^#\d+$/i.test(term)) && !DRIVE_SEARCH_STOP_WORDS.has(term.toLowerCase()))
    .slice(0, 6);
  const searchQuery = terms.length
    ? ` and (${terms.map(term => {
        const safe = escapeDriveQueryValue(term);
  ...
  const listOptions = {
    q:        `${baseQuery}${searchQuery}`,
    fields:   "files(id, name, size, modifiedTime)",
    pageSize: 200,
  };
  if (!terms.length || !includeFullText) listOptions.orderBy = "name";

  const res = await drive.files.list(listOptions);
  return res.data.files ?? [];
}

export async function streamDriveFile(drive, fileId) {
