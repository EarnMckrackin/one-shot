...
export async function saveLocalPdf(comicId, file) {
  return saveLocalPdfBlob(comicId, file, file.name);
}

export async function saveLocalPdfBlob(comicId, blob, name = "comic.pdf") {
  const native = await writeNativePdf(comicId, blob).catch(() => null);
